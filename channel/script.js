...
function loading() {
    setTimeout(async () => {
        ...
        for (let i=1;i<responses.length;i++) {comment+=`<div><b style="color:${responses[i][3]};">${responses[i][2]}</b><span> ${responses[i][0].slice(0,-3)} </span></div><code>${responses[i][4]}</code><br><br>`;};
