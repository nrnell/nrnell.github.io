...
function loading() {
    setTimeout(async () => {
        ...
        for (let i=1;i<responses.length;i++) {comment+=`<br><b style="color:${responses[i][3]};">${responses[i][2]}</b><span style="color:#949BA4;">${responses[i][0].slice(0,-3)}</span><pre>${responses[i][4]}</pre>`;};
